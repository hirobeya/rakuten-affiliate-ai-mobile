...
const TOKEN='generic-prod-smoke-20260923-final2';
const KEYWORDS=new Set(['収納ボックス','電気ケトル','USB-C ハブ','Tシャツ','バイクグローブ','野球グローブ']);
async function searchOne(keyword){
  ...
  const j=await r.json(); const item=(j.items||j.Items||[])[0];
...
module.exports=async function(req,res){
  ...
  try{
    ...
    if(!apiKey) return res.status(503).json({ok:false,message:'GROQ_API_KEY missing'});
    ...
    return res.status(200).json({ok:true,keyword,itemName,validation:result.validation,stages:result.stages,rateLimit:result.ai?.rateLimit||{}});
  }catch(error){
    return res.status(500).json({ok:false,keyword,message:String(error?.message||error)});
  }
};