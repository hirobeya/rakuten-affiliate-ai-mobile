...
function validateFeature(feature,itemName,itemCaption){
  ...
  const valid=Boolean(text&&evidenceValid&&numbersValid&&!promoRisk&&!claimRisk&&!genericRisk);
  return {
    text,source,evidence,valid,evidenceValid,numbersValid,promoRisk,claimRisk,genericRisk,
...
function validateSellingPoint(point,itemName,itemCaption){
  ...
  const text=norm(p.text).slice(0,40);
  const source=String(p.source||'');
  const evidence=norm(p.evidence).slice(0,80);
  const validSource=source==='itemName'||source==='itemCaption';
  const evidenceValid=validSource&&evidenceExists(source,evidence,itemName,itemCaption);
  const numbersValid=numbersSupported(text,evidence);
  const promoRisk=PROMO_RE.test(text)||PROMO_RE.test(evidence);
  ...
  const completePhrase=Boolean(text)&&!/(?:[、,・\/]|を|が|に|へ|と|で|て|の|や|から|まで|対)$/.test(text);
  const valid=Boolean(text&&evidenceValid&&textEvidenceValid&&completePhrase&&numbersValid&&meaningSupported&&!promoRisk&&!claimRisk);
  return {
    text,source,evidence,valid,evidenceValid,textEvidenceValid,completePhrase,numbersValid,meaningSupported,
    meaningCoverage:Number(meaningCoverage.toFixed(3)),promoRisk,claimRisk,
    eligibleForPost:valid&&(source==='itemName'||source==='itemCaption')
  };
}

const DERIVED_VALUE_RISK_RE=/絶対|必ず|確実|劇的|最強|最高|完璧|万能|失敗しない|売れる|儲かる|治る|改善|予防|防止|安全|安心|無害|保証|快眠|健康|痩せ|若返|リフトアップ/i;
const DERIVED_GENERIC_RE=/^(?:使いやすい|便利|おすすめ|選びやすい|商品ページ(?:を)?確認|チェックしたい|比較したい)[。！!]*$/;

function validateDerivedValue(value,itemName,itemCaption){
  const p=value&&typeof value==='object'?value:{};
  const text=norm(p.text).slice(0,56);
  const source=String(p.source||'');
  const evidence=norm(p.evidence).slice(0,90);
  const validSource=source==='itemName'||source==='itemCaption';
  const evidenceValid=validSource&&evidenceExists(source,evidence,itemName,itemCaption);
  const numbersValid=numbersSupported(text,evidence);
  const promoRisk=PROMO_RE.test(text)||PROMO_RE.test(evidence);
  const claimRisk=CLAIM_RE.test(text)||CLAIM_RE.test(evidence)||DERIVED_VALUE_RISK_RE.test(text);
...
function validateAiExtraction(raw,input,{imageAvailable=false}={}){
  ...
  const features=Array.isArray(raw?.features)?raw.features.map(f=>validateFeature(f,itemName,itemCaption)):[];  const audienceHook=raw?.audienceHook?validateDerivedValue(raw.audienceHook,itemName,itemCaption):null;  const fitLine=raw?.fitLine?validateDerivedValue(raw.fitLine,itemName,itemCaption):null;
...
function featureValueCandidate(feature,productType){
  const text=norm(feature?.text);
  const source=String(feature?.source||'');
  const evidence=norm(feature?.evidence);
  if(!text||!source||!evidence) return null;
  const make=(hook,benefit)=>({
    hook:{text:hook,source,evidence},
    benefit:{text:benefit,source,evidence},
    feature:text
  });
  if(/タッチ|スマホ対応|スマートフォン対応/i.test(text))
    return make('スマホを見るたびに手袋を外す手間を減らしたい方に','タッチ対応なので、着用したままスマホを操作しやすいです');
  if(/面ファスナー|ベルクロ|フィット調整|調整可能/.test(text))
    return make('手首まわりのフィット感を自分に合わせたい方に','留め具でフィット感を調整しやすい仕様です');
  if(/通気|メッシュ/.test(text))
    return make('長時間使うときの蒸れが気になる方に','通気性を考えた仕様で、熱がこもりにくい構造です');
  if(/折りたたみ|折畳|折畳み/.test(text))
    return make('使わないときの置き場所をできるだけ取りたくない方に','折りたたみ式なので、使わないときはコンパクトにまとめやすいです');
  if(/大容量|収納/.test(text))
    return make('散らかりやすい物をひとまとめにしたい方に','収納スペースを使って、よく使う物をまとめて入れやすいです');
  if(/座れる|腰掛け|ベンチ/.test(text))
    return make('収納だけでなく、ちょっと腰掛けられる家具も欲しい方に','収納と腰掛けの2役で使いやすい仕様です');
  if(/右投げ|左投げ/.test(text))
    return make('利き手に合う野球グローブを選びたい方に',text+'の表記があり、利き手に合わせて選べます');
  if(/軟式|硬式/.test(text))
    return make('プレーする球種に合うグローブを選びたい方に',text+'用として選べる仕様です');
  if(/本革|山羊革|牛革|レザー/.test(text))
    return make('素材感を重視して選びたい方に',text+'を使った仕様です');
  if(/軽量|軽い/.test(text))
    return make('持ち運びや取り回しの負担を抑えたい方に','軽さを意識して選びやすい仕様です');
  if(/防水|撥水/.test(text))
    return make('水まわりや雨の日の使いやすさを重視したい方に',text+'仕様です');
  if(/\d+\s*(?:個|枚|本|点|食|包|セット)/.test(text))
    return make('まとめて揃えておきたい方に',text+'のセット内容です');
  return null;
}

function buildDerivedCopy(validation){
  const productType=validation?.productType?.valid?validation.productType.value:'';
  const features=(validation?.features||[]).filter(x=>x?.eligibleForPost&&x?.text&&x?.evidence);
  for(const feature of features){
    const candidate=featureValueCandidate(feature,productType);
    if(!candidate) continue;
    const hook=validateDerivedValue(candidate.hook,'',feature.source==='itemCaption'?feature.evidence:feature.evidence);
    const benefit=validateDerivedValue(candidate.benefit,'',feature.source==='itemCaption'?feature.evidence:feature.evidence);
    // validateDerivedValue needs evidence to exist in its declared source. Revalidate against the feature evidence as source text.
    const hookChecked=validateDerivedValue({...candidate.hook,source:'itemCaption'},'',feature.evidence);
    const benefitChecked=validateDerivedValue({...candidate.benefit,source:'itemCaption'},'',feature.evidence);
    if(hookChecked.valid&&benefitChecked.valid){
      return {valid:true,hook:hookChecked.text,benefits:[benefitChecked.text],feature:candidate.feature};
    }
  }
  return {valid:false,hook:'',benefits:[],feature:''};
}

function buildValueFirstPost({validation,derived,itemPrice=0}={}){
  if(!(validation?.mode==='simple'||validation?.mode==='simple_partial')) return '';
  const productType=validation?.productType?.valid?norm(validation.productType.value):'';
  const facts=(validation?.features||[])
    .filter(x=>x?.eligibleForPost&&x?.text)
    .map(x=>norm(x.text)).filter(Boolean).filter((x,i,a)=>a.indexOf(x)===i).slice(0,3);
  if(!productType||!facts.length) return '';
  const lines=[];
  if(derived?.valid&&derived.hook&&derived.benefits?.length){
    lines.push(derived.hook.replace(/[。！!]+$/,'')+'。','',derived.benefits[0].replace(/[。！!]+$/,'')+'。','');
  }else{
    lines.push(productType+'を選ぶときに確認したいポイントです。','');
  }
  lines.push('商品の特徴👇',...facts.map(x=>'✔ '+x));
...
module.exports={
  ...
  productTypeCoverage,validateProductType,validateFeature,validateSellingPoint,validateDerivedValue,validateAiExtraction,buildDerivedCopy,buildValueFirstPost,phase1Post,
