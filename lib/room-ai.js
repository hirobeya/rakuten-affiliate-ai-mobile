...
function validateAiExtraction(raw,input,{imageAvailable=false}={}){
  ...
  const invalidCount=features.filter(f=>!f.valid).length;
  ...
  return {
    ...
    featureValidation:{count:featureCount,invalidCount,invalidRatio:Number(invalidRatio.toFixed(3)),partialAllowed:mode==='simple_partial'},
