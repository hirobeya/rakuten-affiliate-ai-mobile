...
function validateAiExtraction(raw,input,{imageAvailable=false}={}){
  ...
  const audienceHook=raw?.audienceHook?validateDerivedValue(raw.audienceHook,itemName,itemCaption):null;
  const buyerBenefits=Array.isArray(raw?.buyerBenefits)?raw.buyerBenefits.map(p=>validateDerivedValue(p,itemName,itemCaption)):[];
  const fitLine=raw?.fitLine?validateDerivedValue(raw.fitLine,itemName,itemCaption):null;
  ...
  return {
    mode,reasons,confidence,mediumHandling:'fallback_fixed',productType,features,sellingPoints,audienceHook,buyerBenefits,fitLine,
...
module.exports={
  ...
  productTypeCoverage,validateProductType,validateFeature,validateSellingPoint,validateDerivedValue,validateAiExtraction,phase1Post,
