...
function validateFeature(feature,itemName,itemCaption){
  ...
  const valid=Boolean(text&&evidenceValid&&textEvidenceValid&&numbersValid&&!promoRisk&&!claimRisk);
  return {
    text,source,evidence,valid,evidenceValid,textEvidenceValid,numbersValid,promoRisk,claimRisk,
...
function validateSellingPoint(point,itemName,itemCaption){
  ...
  const textEvidenceValid=validSource&&evidenceExists(source,text,itemName,itemCaption);
  const completePhrase=Boolean(text)&&!/(?:[、,・\/]|を|が|に|へ|と|で|て|の|や|な|は|も|ため?|から|まで|対)$/.test(text);
