...
function validateProductType(productType,itemName,itemCaption){
  ...
  return {
    ...
    valid:Boolean(value&&evidenceValid&&textEvidenceValid&&numbersValid&&meaningSupported&&!promoRisk&&!claimRisk),
    evidenceValid,
    textEvidenceValid,
    numbersValid,
    meaningSupported,
...
function validateFeature(feature,itemName,itemCaption){
  ...
  return {
    text,source,evidence,valid,evidenceValid,textEvidenceValid,numbersValid,promoRisk,claimRisk,evidencePromoRisk,evidenceClaimRisk,genericRisk,truncationRisk,
...
function validateSellingPoint(point,itemName,itemCaption){
  ...
  const tc=compact(text),ec=compact(evidence);
  const truncationRisk=Boolean(text.length>=60&&tc&&ec.startsWith(tc)&&ec.length>tc.length);
  ...
  return {
    ...
    meaningCoverage:Number(meaningCoverage.toFixed(3)),promoRisk,claimRisk,evidencePromoRisk,evidenceClaimRisk,truncationRisk,
    eligibleForPost:valid&&(source==='itemName'||source==='itemCaption')
  };
}
...
module.exports={
  ...
  productTypeCoverage,validateProductType,validateFeature,validateSellingPoint,validateAiExtraction,phase1Post,
