...
async function stripeGet(path) {
  const c = config();
  const r = await fetch('https://api.stripe.com/v1' + path, {headers:{Authorization:`Bearer ${c.key}`, 'Stripe-Version':'2026-07-29.dahlia'},signal:AbortSignal.timeout(12000)});
...
async function syncSubscription(subscriptionId) {
  // Record observation BEFORE fetching. The database rejects an older concurrent observation.
...
async function activate(sessionId) {
  ...
  if (!row?.active) throw new Error('Subscription inactive');
...
module.exports = {config,db,stripeGet,subscriptionState,syncSubscription,ensureAuthUser,activate,authorize,verifySignature};
