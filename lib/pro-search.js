...
async function searchProducts(settings,{excludeCodes=[]}={}){
  ...
  const theme=String(settings.theme||'').trim().slice(0,128);
  const productKeyword=String(settings.product_keyword||'').trim().slice(0,128);
  const genreId=String(settings.genre_id||'').trim();
  const genreName=String(settings.genre_name||'').trim();
  const sort=String(settings.sort||'standard');
  const p=new URLSearchParams({applicationId:appId,affiliateId,accessKey,format:'json',formatVersion:'2',hits:'30',availability:'1',sort:'standard'});
  if(/^\d+$/.test(genreId)) p.set('genreId',genreId);
  if(productKeyword) p.set('keyword',productKeyword);
  if(!p.has('genreId')&&!p.has('keyword')) p.set('keyword',String(settings.genre||'便利グッズ').trim().slice(0,128));
  ...
  const ranked=raw.map(x=>{
    const rel=productKeyword ? relevance(x,productKeyword) : (theme ? relevance(x,theme) : (genreName ? 70 : 50));
