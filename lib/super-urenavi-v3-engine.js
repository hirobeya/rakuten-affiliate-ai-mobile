...
async function analyzeProductV3({
  ...
  if(isV3Row(row)){
    ...
    await store.saveProduct({...keyArgs,model:pass2Response?.model||row.model||model,promptVersion:pass2PromptVersion,schemaVersion:V3_SCHEMA_VERSION,rawAiJson:combined,resultStatus:'ok'});
  ...
  if(!validation.valid){
    ...
    await store.saveProduct({...keyArgs,model:pass1Response?.model||model,promptVersion,schemaVersion:V3_SCHEMA_VERSION,rawAiJson:raw,resultStatus:'unknown'});
  ...
  if(!needsPass2(validation)){
    const raw={pass1:pass1Raw,pass2:null};
    await store.saveProduct({...keyArgs,model:pass1Response?.model||model,promptVersion,schemaVersion:V3_SCHEMA_VERSION,rawAiJson:raw,resultStatus:'ok'});
  ...
  // Save pass 1 before pass 2 so a temporary limit/error never forces a second understanding call.
  const partial={pass1:pass1Raw,pass2:null};
  await store.saveProduct({...keyArgs,model:pass1Response?.model||model,promptVersion,schemaVersion:V3_SCHEMA_VERSION,rawAiJson:partial,resultStatus:'ok'});
  ...
  await store.saveProduct({...keyArgs,model:pass2Response?.model||pass1Response?.model||model,promptVersion:pass2PromptVersion,schemaVersion:V3_SCHEMA_VERSION,rawAiJson:combined,resultStatus:'ok'});
...
module.exports={V3_SCHEMA_VERSION,isV3Row,needsPass2,analyzeProductV3};
