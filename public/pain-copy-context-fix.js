(function(root){
  ...
  function titleRoleContext(name){
    ...
    if(/背中/.test(t) && /(?:薬|軟膏|クリーム|ローション|日焼け止め|塗る|塗布)/.test(t) && /(?:道具|器具|アプリケーター|スティック|ヘラ|パッド|ローラー|塗り)/.test(t)) return {
      hook:'背中に薬やクリームを塗りたい。でも手が届きにくい場所はひとりだと大変。',
      bridge:'手が届きにくい背中へ、薬・軟膏・クリームなどを自分で塗るときに使う補助アイテム。',
      points:['手が届きにくい背中に塗りたい','ひとりでも背中のケアをしやすくしたい','薬やクリームを塗る動作を補助したい'],
      benefit:'手が届きにくい背中へ塗る動作を補助しやすい',
      audience:'背中に薬やクリームを自分で塗りたい人',
      _role:'back-applicator',
      _confidence:12
    };

    if(/網戸|あみ戸|アミ戸/.test(t) && /掃除|清掃|クリーナー|ブラシ|ワイパー|モップ|スポンジ|びっクリーン/.test(t)) return {
      hook:'網戸の汚れ、外して洗うのは大がかり。できればそのままサッと掃除したい。',
      bridge:'網戸を外さず、気づいたときに手早く汚れを落としたい人向けの掃除アイテム。',
      points:['網戸を外さず掃除したい','細かい網目の汚れを手早く落としたい','大がかりな掃除の手間を減らしたい'],
      benefit:'網戸掃除を手軽に続けやすい',
      audience:'網戸を外さず手軽に掃除したい人'
    };
    ...
    if(/(?:掃除|お掃除).*(?:手袋|グローブ|ミトン)|(?:手袋|グローブ|ミトン).*(?:掃除|お掃除)|マイクロファイバー.*(?:手袋|グローブ|ミトン)|(?:手袋|グローブ|ミトン).*マイクロファイバー/.test(t)) return {
      hook:'棚や家具の細かい場所、クロスを持ち替えながら拭くのは意外と面倒。',
      bridge:'手にはめてそのままなでるように拭ける掃除グローブなら、凹凸や細かい部分のホコリ取りを進めやすい。',
      points:['手にはめてサッとホコリを取りたい','家具や棚の細かい部分を拭きたい','クロスを持ち替える手間を減らしたい'],
      benefit:'手にはめたまま細かい場所を拭き掃除しやすい',
      audience:'家具や棚の細かい部分を手早く拭きたい人',
      _role:'cleaning-glove',
      _confidence:10
    };

    if(/(?:掃除|お掃除).*(?:ブラシ)|(?:ブラシ).*(?:掃除|お掃除)|掃除ブラシ|清掃ブラシ/.test(t) && !/ヘアブラシ|歯ブラシ/.test(t)) return {
      hook:'溝やすき間に入り込んだ汚れは、クロスだけでは取りにくい。',
      bridge:'掃除ブラシなら、凹凸や細いすき間の汚れをかき出して狙って掃除しやすい。',
      points:['溝やすき間の汚れを落としたい','細かい場所を狙って掃除したい','こびりついた汚れをかき出したい'],
      benefit:'細かいすき間や凹凸の汚れを狙って掃除しやすい',
      audience:'すき間や溝の汚れをしっかり落としたい人',
      _role:'cleaning-brush',
      _confidence:10
    };

    if(/(?:掃除|お掃除).*(?:モップ|ワイパー)|(?:モップ|ワイパー).*(?:掃除|お掃除)|ハンディモップ|フロアワイパー/.test(t)) return {
      hook:'床や高い所のホコリ、気づいても道具を出すのが面倒で後回しになりがち。',
      bridge:'モップやワイパーなら、広い面や手が届きにくい場所をまとめて掃除しやすい。',
      points:['広い面のホコリをまとめて取りたい','床や高い場所を手早く掃除したい','掃除道具をサッと使いたい'],
      benefit:'広い面や手が届きにくい場所を掃除しやすい',
      audience:'床や広い面の掃除を手早く済ませたい人',
      _role:'cleaning-mop',
      _confidence:10
    };

    if(/(?:掃除|お掃除).*(?:クロス|マイクロファイバー)|(?:クロス|マイクロファイバー).*(?:掃除|お掃除)|ほこり吸着|ホコリ吸着/.test(t)) return {
      hook:'ホコリや水滴、気づいたときにサッと拭けるものがあると掃除が後回しになりにくい。',
      bridge:'マイクロファイバーの掃除クロスで、ホコリ取りや水分の拭き取りを手早く済ませたい人向け。',
      points:['ホコリをサッと拭き取りたい','水分や汚れを手早く拭きたい','日常のちょこっと掃除をラクにしたい'],
  ...
  function genericMetadataContext(name,description,genrePath){
    ...
    const contexts={
      measure:{
        tokens:['計量','目盛','メジャー','容量測定','量る','はかる'],
        ctx:{hook:'「だいたい」で済ませたくない。でも、わざわざ別の道具を出すのも面倒。',bridge:'使う動作の中でそのまま量れるなら、準備も洗い物も増やしにくい。',points:['使いながら量りたい','別の計量道具を出す手間を減らしたい','作業を止めずに進めたい'],benefit:'量るためのひと手間を減らしやすい',audience:'計量のひと手間を減らしたい人'}
      },
      pour:{
        tokens:['注ぎ','注ぐ','口付き','横口','液だれ','ドリップ'],
        ctx:{hook:'注ぐたびにこぼれる、垂れる。ほんの少しのことやけど毎回だと気になる。',bridge:'狙ったところへ注ぎやすい形なら、移し替えや後拭きの手間を減らしやすい。',points:['こぼれや液だれを減らしたい','狙った場所へ注ぎたい','移し替えを手早く済ませたい'],benefit:'注ぐときの小さなストレスを減らしやすい',audience:'液体をこぼさず扱いたい人'}
      },
      strainer:{
        tokens:['すくい網','すくいアミ','ストレーナー','ざる','濾し','こし器','かす揚げ','あく取り','アク取り'],
        ctx:{hook:'必要なものだけ取りたいのに、普通のおたまやスプーンではうまくすくえない。',bridge:'細かいものを分けてすくえる道具があると、鍋や揚げ物の作業が止まりにくい。',points:['細かいものだけすくいたい','汁や油を切りながら取りたい','調理中の手間を減らしたい'],benefit:'必要なものだけを手早くすくいやすい',audience:'鍋や揚げ物で細かいものをすくう機会が多い人'}
      },
      cutting:{
        tokens:['カッター','スライス','千切り','みじん切り','ピーラー','皮むき','包丁','ハサミ','はさみ','カット'],
        ctx:{hook:'同じ「切る」作業を何度も繰り返すと、意外と時間を取られる。',bridge:'目的に合った専用道具なら、手作業の回数を減らして仕上がりもそろえやすい。',points:['切る作業を時短したい','同じ作業を何度も繰り返したくない','仕上がりをそろえたい'],benefit:'切る作業を短く安定させやすい',audience:'切る作業の手間を減らしたい人'}
      },
      storage:{
        tokens:['収納','整理','片付け','ラック','ケース','ボックス','ホルダー','スタンド','引き出し','クローゼット','置き場'],
        strong:['収納ボックス','収納ケース','収納ラック','収納棚','衣類収納','隙間収納','すき間収納','クローゼット','引き出し','ワゴン'],
        ctx:{hook:'「あとで片付けよう」が増えると、気づけばまた散らかってる。',bridge:'戻す場所が決まるだけで、片付けは頑張る作業から戻すだけの動作に変えやすい。',points:['物の定位置を作りたい','出し入れをラクにしたい','限られた場所を有効に使いたい'],benefit:'散らかりにくい仕組みを作りやすい',audience:'片付けてもすぐ散らかるのを減らしたい人'}
      },
      cleaning:{
        tokens:['掃除','清掃','汚れ','クリーナー','ブラシ','ワイパー','モップ','スポンジ','洗浄','クロス','マイクロファイバー','ほこり','ホコリ','ダスター','吸水'],
        strong:['お掃除クロス','掃除クロス','マイクロファイバー','ほこり吸着','ホコリ吸着','網戸掃除','フロアワイパー','掃除ブラシ'],
        ctx:{hook:'汚れてからまとめて掃除すると、時間も気力も持っていかれる。',bridge:'気づいたときにすぐ使える道具なら、汚れをため込む前にサッと対処しやすい。',points:['掃除を短時間で済ませたい','汚れをため込みたくない','準備や片付けの手間を減らしたい'],benefit:'掃除を後回しにしにくくなる',audience:'掃除の負担を少しでも減らしたい人'}
      },
      drying:{
        tokens:['乾燥','水切り','速乾','乾かす','ドライ'],
        ctx:{hook:'使ったあとに乾くまで場所を取る。濡れたまま置いておくのも気になる。',bridge:'水切れや乾きやすさを意識した道具なら、使ったあとの片付けまでラクにしやすい。',points:['乾くまでの時間を短くしたい','濡れた物の置き場所に困りたくない','片付けまでスムーズにしたい'],benefit:'使ったあとの「乾かす・片付ける」をラクにしやすい',audience:'水まわりの後片付けをラクにしたい人'}
      },
      charging:{
        tokens:['充電','バッテリー','USB','Type-C','Lightning','急速充電','ワイヤレス充電'],
        ctx:{hook:'使いたいときに電池がない。結局それが一番困る。',bridge:'必要な場所ですぐ充電できれば、残量を気にする時間を減らせる。',points:['充電切れを避けたい','充電待ちを短くしたい','持ち歩きや配線をラクにしたい'],benefit:'「電池が足りない」の不安を減らしやすい',audience:'充電切れを気にせず使いたい人'}
      },
      portable:{
        tokens:['持ち運び','携帯','軽量','コンパクト','ポータブル','折りたたみ','折畳','旅行','外出'],
        ctx:{hook:'便利でも、重い・大きい・持ちにくいと結局使わなくなる。',bridge:'持ち運びやすさまで考えられた物なら、必要な場面へ気軽に持っていきやすい。',points:['持ち歩く負担を減らしたい','使わない時は省スペースにしたい','外出先でも使いたい'],benefit:'必要な時だけサッと持ち出しやすい',audience:'持ち運びやすさを重視したい人'}
      },
      waterproof:{
        tokens:['防水','撥水','防滴','雨','レイン'],
        ctx:{hook:'少し濡れただけで気を遣う物は、雨の日や水まわりでは使いにくい。',bridge:'水への強さがあると、天気や場所を気にせず使える場面が広がる。',points:['濡れる場面でも使いたい','雨の日のストレスを減らしたい','水まわりで気軽に使いたい'],benefit:'濡れを気にする場面を減らしやすい',audience:'雨や水ぬれを気にせず使いたい人'}
      },
      thermal:{
        tokens:['保温','保冷','冷たい','温かい','温度','断熱'],
        ctx:{hook:'せっかく用意したのに、使う頃には温度が変わってる。',bridge:'ちょうどいい温度を長く保てれば、時間を気にせず使いやすい。',points:['温度をできるだけ保ちたい','外出先でも使いたい','使い頃を長くしたい'],benefit:'ちょうどいい温度を保ちやすい',audience:'温度を保って使いたい人'}
      },
      deodorize:{
        tokens:['消臭','防臭','臭い','ニオイ','におい','脱臭'],
        ctx:{hook:'見た目は片付いていても、ニオイが残るとそれだけで気になる。',bridge:'毎回大がかりに対策せず、普段の流れの中でニオイを抑えたい人向け。',points:['気になるニオイを減らしたい','手軽に対策を続けたい','生活臭のストレスを減らしたい'],benefit:'ニオイ対策を日常に取り入れやすい',audience:'生活の中の気になるニオイを減らしたい人'}
      },
      convenience:{
        tokens:['時短','簡単','手軽','ラク','らく','ワンタッチ','自動','一発','すぐ'],
        weak:true,
        ctx:{hook:'数分で済む作業でも、毎日繰り返すと意外と大きな手間になる。',bridge:'工程をひとつ減らせる道具は、派手じゃなくても日常では効いてくる。',points:['毎日の作業を時短したい','面倒な手順を減らしたい','簡単に使い続けたい'],benefit:'毎日の小さな手間を積み重ねて減らしやすい',audience:'日常のちょっとした面倒を減らしたい人'}
      }
    };

    const countHits=(text,tokens)=>tokens.reduce((n,token)=>n+(text.includes(token)?1:0),0);
    ...
    const core=t
      .replace(/【[^】]*】|\[[^\]]*\]|［[^］]*］/g,' ')
      .replace(/送料無料|公式|正規品|ランキング|クーポン|ポイント|SALE|セール/gi,' ')
      .replace(/\s+/g,' ')
      .trim()
      .slice(0,28);
